...
class WelcomePage extends React.Component {
  ...
  render() {
    return (
      <div>
        <div className="welcomePage__welcome">Welcome to your Personal Space</div>
  ...
  loginUser = (event) => {
    ...
    axios
    .post('https://secure-taiga-54603.herokuapp.com/api/login', { name, password })
    .then(res => {
  ...
  registerUser = (event) => {
    ...
    axios
      .post('https://secure-taiga-54603.herokuapp.com/api/users', { name, password })
      .then(res => {
        this.changeMessage(res.data.message);
        this.cancel();
      })
